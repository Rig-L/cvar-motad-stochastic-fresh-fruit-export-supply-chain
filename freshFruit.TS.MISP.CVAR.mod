...
#C-VAR Parameters
param aversion:=1; #Risk aversion
param cotaVAR:=0.444; #Risky tail of dsitribution
...
sum{w in W}(prob[w]*(
	...
	aversion*(G-(1/(cotaVAR))*sum{w in W}(prob[w]*V[w]));

#########################################
#C-VAR CONSTRAINTS
s.t. RCVAR{w in W}: 
V[w] >= G - (-sum{t in T, d in D, cl in C} (pc * PP[t,d,cl])
        - sum{t in T, z in Z} (tcz[z] * NT[z,t])
        - (sp * PW)
        - sum{t in T} (st * TW[t])
        - sum{t in T, d in D, cl in C} (IUP[t,d,cl] + ITP[t,d,cl]) * ic
